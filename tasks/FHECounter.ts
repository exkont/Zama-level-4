...
/**
 * Example:
 *   - npx hardhat --network local task:faucet
 */
task("task:faucet", "Fund the first 5 accounts derived from the mnemonic with coins").setAction(async function (_taskArguments: TaskArguments, hre) {
  const { ethers } = hre;
  const signers = await ethers.getSigners();
  const faucetSigner = signers[0];

  // Fund the first 5 accounts with 100 ETH each
  for (let i = 1; i < Math.min(6, signers.length); i++) {
    const tx = await faucetSigner.sendTransaction({
      to: signers[i].address,
      value: ethers.parseEther("100")
    });
    await tx.wait();
    console.log(`✅ Account ${i} (${signers[i].address}) funded with 100 ETH`);
  }
